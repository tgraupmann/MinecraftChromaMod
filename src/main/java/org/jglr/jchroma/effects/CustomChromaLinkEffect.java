 ...
 */
public class CustomChromaLinkEffect extends ChromaLinkEffect implements ChromaEffect1D {
    public static final int MAX_LEDS = 5;
    private final CustomStructure struct;

    /**
     * Creates a custom chromalink effect with colors filled with <code>ColorRef.NULL</code>. One should rewrite the value of the array instead of using ColorRef::setRed (or equivalent)
     */
    ...
    /**
     * Creates a custom chromalink effect with already assigned colors
     * @param colors
     *      The colors to assign to each key (in row/column order)
     */
    public CustomChromaLinkEffect(ColorRef[] colors) {
        super();
    ...
    @Override
    public ChromaLinkEffectType getType() {
        return ChromaLinkEffectType.CUSTOM;
    }
